//
// S6 from dox/HASHMAP2.md: remove racing the freeze.
//
...
// a probe thread calls _mulle_concurrent_hashmap2_migrate_same_size() in a tight
...
int   main( void)
{
   ...
   mulle_concurrent_hashmap2_init( &g_map, 256, NULL);

   // some unrelated live entries, so copy has real work to do on every pass
   for( i = 0; i < OTHER_KEYS; i++)
      mulle_concurrent_hashmap2_insert( &g_map, i + 1000, (void *)(uintptr_t)(i + 1));
   ...
   for( i = 0; i < OTHER_KEYS; i++)
      if( mulle_concurrent_hashmap2_lookup( &g_map, i + 1000) != (void *)(uintptr_t)(i + 1))
      {
